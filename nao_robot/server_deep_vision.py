...
def get_frame(source="webcam"):
    if source == "webcam":
        ...
        try:
            # Run subprocess to fetch image from NAO (NAOqi is compatible with Python 2.7)
            output = subprocess.check_output(
                [".venv/Scripts/python.exe", "get_camera_deep_vision.py"],
                stderr=subprocess.STDOUT
            )
            # Decode output from "get_camera_deep_vision.py"
...
while True:
    ...
    # Skip frames to boost FPS   # frame_count += 1
    # if frame_count % 2 != 0:
    #     # Skip this frame
    #     continue
    ...
    # Determine if any class meets threshold (confidence > 4/5)
    ...
    # Send TTS output to NAO robot
    if confirmed_label and recall != confirmed_label:
        recall = confirmed_label
        try:
            subprocess.Popen([
                ".venv/Scripts/python.exe", "speak_deep_vision.py", confirmed_label
            ])
        except Exception as e:
